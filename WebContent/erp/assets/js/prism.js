/* http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+json&plugins=file-highlight */
		...
		tokenloop: for (var token in grammar) {
			...
			for (var j = 0; j < patterns.length; ++j) {
				...
				if (greedy && !pattern.pattern.global) {
					...
					// @MODIFICATION START
					// fix for when flags are undefined
...
if (script) {
	...
	if (document.addEventListener && !script.hasAttribute('data-manual')) {
		if(document.readyState !== "loading") {
			// @MODIFICATION START
			// fix: check if window.requestAnimationFrame exists
