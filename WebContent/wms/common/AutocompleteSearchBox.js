﻿﻿angular.module('MetronicApp').directive('autoSearch', function ($rootScope,RestEndpoint, CommonService, gettextCatalog, $http) {
    ...
    return {
		...
		template: '<div class="form-group col-md-6" id="{{iddiv}}" '+
			//hainh49 start 19/5/2020
			'disable-all="disabledEdit">' +
			//hainh49 end
			'<label id="{{idlable}}" translate class="col-md-4 control-label {{req}}">{{modelLabel}}</label>' +
			'<div class="col-md-8">' +
...
//					var title = gettextCatalog.getString(label);
					...
					vm.gridOptions = kendoConfig.getGridOptions({
						...
						dataSource: {
							...
							transport: {
					            read: {
					               url: RestEndpoint.BASE_SERVICE_URL + "sysUserRsServiceRest/doSearchUserInPopup",
					               contentType: "application/json; charset=utf-8",
					               type: "POST"
					            },
